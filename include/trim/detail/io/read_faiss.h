...
InvertedLists* trim_read_InvertedLists(IOReader* f, int io_flags) {
  uint32_t h;
  READ1(h);
  if (h == fourcc("il00")) {
    fprintf(stderr,
            "trim_read_InvertedLists:"
    ...
    trim_read_ArrayInvertedLists_sizes(f, sizes);
...
void trim_read_InvertedLists(IndexIVF* ivf, IOReader* f, int io_flags) {
  InvertedLists* ils = trim_read_InvertedLists(f, io_flags);
  if (ils) {
...
ProductQuantizer* trim_read_ProductQuantizer(IOReader* reader) {
  ...
  trim_read_ProductQuantizer(pq, reader);
  del.release();
  return pq;
}
...
ProductQuantizer* trim_read_ProductQuantizer(const char* fname) {
  ...
  return trim_read_ProductQuantizer(&reader);
}
...
void trim_read_ivf_header(IndexIVF* ivf, IOReader* f, std::vector<std::vector<idx_t>>* ids) {
  trim_read_index_header(ivf, f);
  READ1(ivf->nlist);
  READ1(ivf->nprobe);
  ivf->quantizer = trim_read_index(f);
  ivf->own_fields = true;
  if (ids) {  // used in legacy "Iv" formats
    ids->resize(ivf->nlist);
    for (size_t i = 0; i < ivf->nlist; i++) READVECTOR((*ids)[i]);
  }
  trim_read_direct_map(&ivf->direct_map, f);
}
...
static IndexIVFPQ* trim_read_ivfpq(IOReader* f, uint32_t h, int io_flags) {
  ...
  if (legacy) {
    ArrayInvertedLists* ail = trim_set_array_invlist(ivpq, ids);
...
int trim_read_old_fmt_hack = 0;

Index* trim_read_index(IOReader* f, int io_flags) {
  Index* idx = nullptr;
  uint32_t h;
  READ1(h);
  if (h == fourcc("null")) {
    ...
    trim_read_index_header(idxp, f);
    trim_read_ProductQuantizer(&idxp->pq, f);
    ...
    trim_read_index_header(idxmap, f);
    idxmap->index = trim_read_index(f, io_flags);
...
Index* trim_read_index(FILE* f, int io_flags) {
  FileIOReader reader(f);
  return trim_read_index(&reader, io_flags);
}

Index* trim_read_index(const char* fname, int io_flags) {
  FileIOReader reader(fname);
