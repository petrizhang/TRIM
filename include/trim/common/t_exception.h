...
/// Base class for TRIM exceptions
class TrimException : public std::exception {
 public:
  explicit TrimException(const std::string& msg);

  TrimException(const std::string& msg, const char* funcName, const char* file, int line);
...
void trim_handle_exceptions(std::vector<std::pair<int, std::exception_ptr>>& exceptions);

/// make typeids more readable
std::string trim_demangle_cpp_symbol(const char* name);

}  // namespace trim

namespace trim {

TrimException::TrimException(const std::string& m) : msg(m) {}

TrimException::TrimException(const std::string& m, const char* funcName, const char* file,
                               int line) {
...
const char* TrimException::what() const noexcept { return msg.c_str(); }
