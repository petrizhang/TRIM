...
struct Object {
  ...
  bool get_bool() const {
    T_ASSERT(type == BOOL_TYPE);
    return std::any_cast<bool>(value);
  }

  int64_t get_int64() const {
    T_ASSERT(type == INTEGER_TYPE);
    return std::any_cast<int64_t>(value);
  }

  double get_double() const {
    T_ASSERT(type == DOUBLE_TYPE);
    return std::any_cast<double>(value);
  }

  std::string get_string() const {
    T_ASSERT(type == STRING_TYPE);
  ...
  std::string to_string() const {
    switch (this->type) {
      ...
      case STRING_TYPE:
        return get_string();
      default:
        throw trim::TrimException("unexpected code path");
    }
  }
};
  ...
  std::optional<T> _checked_get(const std::string& key) const {
    ...
    if (it == mapping.end()) {
      if constexpr (is_required) {
        T_THROW_FMT("missing required key `%s`", key.c_str());
      }
      return {};
    }
    ...
    if constexpr (std::is_same_v<T, bool>) {
      T_THROW_IF_NOT_FMT(obj.type == ObjectType::BOOL_TYPE, "`%s` must be a boolean value",
                         key.c_str());
      return obj.get_bool();
    }

    if constexpr (std::is_integral_v<T>) {
      T_THROW_IF_NOT_FMT(obj.type == ObjectType::INTEGER_TYPE, "`%s` must be an integer value",
                         key.c_str());
      return obj.get_int64();
    }

    if constexpr (std::is_floating_point_v<T>) {
      T_THROW_IF_NOT_FMT(obj.type == ObjectType::DOUBLE_TYPE, "`%s` must be a double value",
                         key.c_str());
      return obj.get_double();
    }

    if constexpr (std::is_same_v<T, std::string>) {
      T_THROW_IF_NOT_FMT(obj.type == ObjectType::STRING_TYPE, "`%s` must be a string", key.c_str());
      return obj.get_string();
    }

    T_THROW_MSG("reading unsupported type from Dict");
  }
};
