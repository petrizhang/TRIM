...
std::unique_ptr<TrimHNSW> read_uhnsw(const Dict& options) {
  ...
  if (opt_pq_index_path.has_value()) {
    ...
    uhnsw->trim_index_pq = TrimIndexPQ(std::move(faiss_index_pq), faiss_index_pq.get());

    // Rorder PQ codes
    uhnsw->reorder_pq_codes();
    // Compute PQ reconstruction errors
    ExactDCO<> dco(uhnsw->owned_index_hnsw.get());
    uhnsw->trim_index_pq.compute_pq_reconstruction_errors(&dco, pool);
  }
...
std::unique_ptr<ISearcher> create_hnsw_searcher(const Dict& options) {
  ...
  std::string dco_type =
      options.optional<std::string>(constants::T_DCO).value_or(constants::T_DCO_TRIM);
  ...
  if (dco_type == constants::T_DCO_EXACT) {
    ...
    T_THROW_IF_NOT_MSG(index->trim_index_pq.index_pq != nullptr,
