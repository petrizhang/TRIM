...
#include "top/detail/core/object.h"
...
struct Searcher {
  ...
  Searcher(const Graph& graph, py::object input, const std::string& metric, int level)
      : searcher(std::unique_ptr<top::Searcher>(top::create_searcher(graph.graph, metric, level))) {
...
PYBIND11_MODULE(topnn, m) {
  ...
  py::class_<Searcher>(m, "Searcher")
      .def(py::init<const Graph&, py::object, const std::string&, int>(), py::arg("graph"),
           py::arg("data"), py::arg("metric"), py::arg("level"))
