...
#include <unity/detail/faiss/impl/FaissAssert.h>
#include <unity/detail/faiss/impl/platform_macros.h>
#include <unity/detail/faiss/utils/simdlib.h>
...
#include <unity/detail/faiss/utils/transpose/transpose-avx512-inl.h>
#elif defined(__AVX2__)
#include <unity/detail/faiss/utils/transpose/transpose-avx2-inl.h>
#endif
...
inline float fvec_L1_ref(const float* x, const float* y, size_t d) {
  size_t i;
  float res = 0;
  for (i = 0; i < d; i++) {
...
inline float fvec_Linf_ref(const float* x, const float* y, size_t d) {
  size_t i;
  float res = 0;
  for (i = 0; i < d; i++) {
...
inline void fvec_L2sqr_ny_ref(float* dis, const float* x, const float* y, size_t d, size_t ny) {
...
inline void fvec_L2sqr_ny_y_transposed_ref(float* dis, const float* x, const float* y,
...
inline size_t fvec_L2sqr_ny_nearest_ref(float* distances_tmp_buffer, const float* x, const float* y,
                                 size_t d, size_t ny) {
...
inline size_t fvec_L2sqr_ny_nearest_y_transposed_ref(float* distances_tmp_buffer, const float* x,
                                              const float* y, const float* y_sqlen, size_t d,
                                              size_t d_offset, size_t ny) {
  fvec_L2sqr_ny_y_transposed_ref(distances_tmp_buffer, x, y, y_sqlen, d, d_offset, ny);
...
inline void fvec_inner_products_ny_ref(float* ip, const float* x, const float* y, size_t d, size_t ny) {
...
/// between x and yi
FAISS_PRAGMA_IMPRECISE_FUNCTION_BEGIN
inline void fvec_inner_product_batch_4(const float* __restrict x, const float* __restrict y0,
                                const float* __restrict y1, const float* __restrict y2,
                                const float* __restrict y3, const size_t d, float& dis0,
...
FAISS_PRAGMA_IMPRECISE_FUNCTION_BEGIN
inline void fvec_L2sqr_batch_4(const float* x, const float* y0, const float* y1, const float* y2,
...
template <>
void fvec_op_ny_D2<ElementOpIP>(float* dis, const float* x, const float* y, size_t ny) {
...
template <>
void fvec_op_ny_D2<ElementOpL2>(float* dis, const float* x, const float* y, size_t ny) {
  const size_t ny16 = ny / 16;
  size_t i = 0;
...
template <>
void fvec_op_ny_D4<ElementOpIP>(float* dis, const float* x, const float* y, size_t ny) {
  const size_t ny16 = ny / 16;
  size_t i = 0;
  ...
  if (ny16 > 0) {
    ...
    for (i = 0; i < ny16 * 16; i += 16) {
      ...
      transpose_16x4(_mm512_loadu_ps(y + 0 * 16), _mm512_loadu_ps(y + 1 * 16),
                     _mm512_loadu_ps(y + 2 * 16), _mm512_loadu_ps(y + 3 * 16), v0, v1, v2, v3);
...
template <>
void fvec_op_ny_D4<ElementOpL2>(float* dis, const float* x, const float* y, size_t ny) {
  const size_t ny16 = ny / 16;
  size_t i = 0;
...
void fvec_op_ny_D4<ElementOpIP>(float* dis, const float* x, const float* y, size_t ny) {
  ...
  if (ny8 > 0) {
    ...
    for (i = 0; i < ny8 * 8; i += 8) {
      ...
      transpose_8x4(_mm256_loadu_ps(y + 0 * 8), _mm256_loadu_ps(y + 1 * 8),
                    _mm256_loadu_ps(y + 2 * 8), _mm256_loadu_ps(y + 3 * 8), v0, v1, v2, v3);
...
template <>
void fvec_op_ny_D8<ElementOpIP>(float* dis, const float* x, const float* y, size_t ny) {
  const size_t ny16 = ny / 16;
  size_t i = 0;
  ...
  if (ny16 > 0) {
    ...
    for (i = 0; i < ny16 * 16; i += 16) {
      ...
      transpose_16x8(
          _mm512_loadu_ps(y + 0 * 16), _mm512_loadu_ps(y + 1 * 16), _mm512_loadu_ps(y + 2 * 16),
...
template <>
void fvec_op_ny_D8<ElementOpL2>(float* dis, const float* x, const float* y, size_t ny) {
  const size_t ny16 = ny / 16;
  size_t i = 0;
...
void fvec_op_ny_D8<ElementOpIP>(float* dis, const float* x, const float* y, size_t ny) {
  ...
  if (ny8 > 0) {
    ...
    for (i = 0; i < ny8 * 8; i += 8) {
      ...
      transpose_8x8(
          _mm256_loadu_ps(y + 0 * 8), _mm256_loadu_ps(y + 1 * 8), _mm256_loadu_ps(y + 2 * 8),
...
inline void fvec_inner_products_ny(float* dis, const float* x, const float* y, size_t d, size_t ny) {
#define DISPATCH(dval)                              \
  case dval:                                        \
...
template <size_t DIM>
void fvec_L2sqr_ny_y_transposed_D(float* distances, const float* x, const float* y,
                                  const float* y_sqlen, const size_t d_offset, size_t ny) {
...
void fvec_L2sqr_ny_transposed(float* dis, const float* x, const float* y, const float* y_sqlen,
...
#define DISPATCH(dval) \
  case dval:           \
    return fvec_L2sqr_ny_y_transposed_D<dval>(dis, x, y, y_sqlen, d_offset, ny);
...
size_t fvec_L2sqr_ny_nearest_D2(float* distances_tmp_buffer, const float* x, const float* y,
                                size_t ny) {
...
inline size_t fvec_L2sqr_ny_nearest_D4(float* distances_tmp_buffer, const float* x, const float* y,
                                size_t ny) {
  // this implementation does not use distances_tmp_buffer.
...
inline size_t fvec_L2sqr_ny_nearest_D8(float* distances_tmp_buffer, const float* x, const float* y,
                                size_t ny) {
  // this implementation does not use distances_tmp_buffer.
...
inline size_t fvec_L2sqr_ny_nearest_D2(float* distances_tmp_buffer, const float* x, const float* y,
                                size_t ny) {
  // this implementation does not use distances_tmp_buffer.
  ...
  if (ny8 > 0) {
    ...
    for (; i < ny8 * 8; i += 8) {
      ...
      min_indices = _mm256_castps_si256(_mm256_blendv_ps(
          _mm256_castsi256_ps(current_indices), _mm256_castsi256_ps(min_indices), comparison));
...
inline size_t fvec_L2sqr_ny_nearest(float* distances_tmp_buffer, const float* x, const float* y, size_t d,
                             size_t ny) {
  // optimized for a few special cases
#define DISPATCH(dval) \
  case dval:           \
...
template <size_t DIM>
size_t fvec_L2sqr_ny_nearest_y_transposed_D(float* distances_tmp_buffer, const float* x,
                                            const float* y, const float* y_sqlen,
                                            const size_t d_offset, size_t ny) {
  // This implementation does not use distances_tmp_buffer.
  ...
  if (ny16 > 0) {
    ...
    for (; i < ny16 * 16; i += 16) {
      ...
      const __m512 distances = _mm512_sub_ps(_mm512_loadu_ps(y_sqlen), dp);

      // Compare the new distances to the min distances
      __mmask16 comparison = _mm512_cmp_ps_mask(min_distances, distances, _CMP_LT_OS);

      // Update min distances and indices with closest vectors if needed
      min_distances = _mm512_mask_blend_ps(comparison, distances, min_distances);
      min_indices = _mm512_castps_si512(_mm512_mask_blend_ps(
          comparison, _mm512_castsi512_ps(current_indices), _mm512_castsi512_ps(min_indices)));
...
size_t fvec_L2sqr_ny_nearest_y_transposed_D(float* distances_tmp_buffer, const float* x,
  ...
  if (ny8 > 0) {
    ...
    for (; i < ny8 * 8; i += 8) {
      ...
      //   lowest distance.
      // x^2 is the constant that can be avoided.
      const __m256 distances = _mm256_sub_ps(_mm256_loadu_ps(y_sqlen), dp);

      // compare the new distances to the min distances
      // for each of 8 AVX2 components.
      const __m256 comparison = _mm256_cmp_ps(min_distances, distances, _CMP_LT_OS);

      // update min distances and indices with closest vectors if needed.
      min_distances = _mm256_blendv_ps(distances, min_distances, comparison);
      min_indices = _mm256_castps_si256(_mm256_blendv_ps(
          _mm256_castsi256_ps(current_indices), _mm256_castsi256_ps(min_indices), comparison));
...
inline size_t fvec_L2sqr_ny_nearest_y_transposed(float* distances_tmp_buffer, const float* x,
                                          const float* y, const float* y_sqlen, size_t d,
                                          size_t d_offset, size_t ny) {
  // optimized for a few special cases
#ifdef __AVX2__
#define DISPATCH(dval)                                                                     \
  case dval:                                                                               \
    return fvec_L2sqr_ny_nearest_y_transposed_D<dval>(distances_tmp_buffer, x, y, y_sqlen, \
                                                      d_offset, ny);
  ...
  switch (d) {
    ...
    DISPATCH(8)
    default:
      return fvec_L2sqr_ny_nearest_y_transposed_ref(distances_tmp_buffer, x, y, y_sqlen, d,
                                                    d_offset, ny);
  }
#undef DISPATCH
#else
  // non-AVX2 case
  return fvec_L2sqr_ny_nearest_y_transposed_ref(distances_tmp_buffer, x, y, y_sqlen, d, d_offset,
                                                ny);
#endif
}
...
inline float fvec_Linf(const float* x, const float* y, size_t d) {
  __m256 msum1 = _mm256_setzero_ps();
  __m256 signmask = _mm256_castsi256_ps(_mm256_set1_epi32(0x7fffffffUL));
...
inline float fvec_L1(const float* x, const float* y, size_t d) { return fvec_L1_ref(x, y, d); }

inline float fvec_Linf(const float* x, const float* y, size_t d) { return fvec_Linf_ref(x, y, d); }

#elif defined(__aarch64__)

// not optimized for ARM
inline void fvec_L2sqr_ny(float* dis, const float* x, const float* y, size_t d, size_t ny) {
  fvec_L2sqr_ny_ref(dis, x, y, d, ny);
}

inline void fvec_L2sqr_ny_transposed(float* dis, const float* x, const float* y, const float* y_sqlen,
                              size_t d, size_t d_offset, size_t ny) {
  return fvec_L2sqr_ny_y_transposed_ref(dis, x, y, y_sqlen, d, d_offset, ny);
}
...
[[maybe_unused]] static inline void fvec_madd_ref(size_t n, const float* a, float bf,
...
static inline void fvec_madd_avx512(const size_t n, const float* __restrict a, const float bf,
...
static inline void fvec_madd_avx2(const size_t n, const float* __restrict a, const float bf,
                                  const float* __restrict b, float* __restrict c) {
  //
  const size_t n8 = n / 8;
...
[[maybe_unused]] static inline void fvec_madd_sse(size_t n, const float* a, float bf,
                                                  const float* b, float* c) {
  n >>= 2;
...
inline int fvec_madd_and_argmin(size_t n, const float* a, float bf, const float* b, float* c) {
  ...
  size_t i;
  {
    ...
    for (i = 0; i < n_simd; i += 4) {
      ...
      iminv = vorrq_u32(vandq_u32(less_than, iv), vandq_u32(vmvnq_u32(less_than), iminv));
  ...
  uint32_t imin;
  {
    ...
    imin = vminvq_u32(
        vorrq_u32(vandq_u32(equals, iminv),
                  vandq_u32(vmvnq_u32(equals), vdupq_n_u32(std::numeric_limits<uint32_t>::max()))));
  }
  for (; i < n; ++i) {
    c[i] = a[i] + bf * b[i];
    if (c[i] < vmin) {
...
void pq2_8cents_table(const simd8float32 centroids[8], const simd8float32 x, float* out, size_t ldo,
                      size_t nout = 4) {
  simd8float32 ips[4];
...
inline simd8float32 load_simd8float32_partial(const float* x, int n) {
...
inline void compute_PQ_dis_tables_dsub2(size_t d, size_t ksub, const float* all_centroids, size_t nx,
                                 const float* x, bool is_inner_product, float* dis_tables) {
  size_t M = d / 2;
  ...
  for (size_t m0 = 0; m0 < M; m0 += 4) {
    ...
    for (int k0 = 0; k0 < ksub; k0 += 8) {
      ...
      for (size_t i = 0; i < nx; i++) {
        ...
        if (is_inner_product) {
          pq2_8cents_table<true>(centroids, xi, dis_tables + (i * M + m0) * ksub + k0, ksub,
                                 m1 - m0);
        } else {
          pq2_8cents_table<false>(centroids, xi, dis_tables + (i * M + m0) * ksub + k0, ksub,
                                  m1 - m0);
        }
      }
    }
...
inline void fvec_sub(size_t d, const float* a, const float* b, float* c) {
  size_t i;
  for (i = 0; i + 7 < d; i += 8) {
...
inline void fvec_add(size_t d, const float* a, const float* b, float* c) {
  size_t i;
  for (i = 0; i + 7 < d; i += 8) {
    simd8float32 ci, ai, bi;
    ai.loadu(a + i);
...
inline void fvec_add(size_t d, const float* a, float b, float* c) {
  size_t i;
  simd8float32 bv(b);
  for (i = 0; i + 7 < d; i += 8) {
    simd8float32 ci, ai, bi;
