...
#ifndef __clang__
inline int __builtin_clzll(uint64_t x) { return (int)__lzcnt64(x); }
#endif
...
#define FAISS_PRAGMA_IMPRECISE_FUNCTION_BEGIN __pragma(float_control(precise, off, push))
#define FAISS_PRAGMA_IMPRECISE_FUNCTION_END __pragma(float_control(pop))
#elif defined(__clang__)
#if defined(__PPC__)
#define FAISS_PRAGMA_IMPRECISE_LOOP _Pragma("clang loop vectorize_width(4) interleave_count(8)")
#define FAISS_PRAGMA_IMPRECISE_FUNCTION_BEGIN _Pragma("float_control(precise, off, push)")
#define FAISS_PRAGMA_IMPRECISE_FUNCTION_END _Pragma("float_control(pop)")
#else
#define FAISS_PRAGMA_IMPRECISE_LOOP _Pragma("clang loop vectorize(enable) interleave(enable)")
 ...
 *******************************************************/
#if !defined(_MSC_VER) && (defined(__BYTE_ORDER__) && (__BYTE_ORDER__ == __ORDER_BIG_ENDIAN__))
