...
#include <unity/detail/faiss/impl/code_distance/code_distance-generic.h>
...
float inline distance_single_code_avx2_pqdecoder8_m4(
    ...
    const float* sim_table, const uint8_t* code) {
  float result = 0;
  ...
  const __m128i mm1 = _mm_cvtsi32_si128(*((const int32_t*)code));
  {
    ...
    __m128 collected = _mm_i32gather_ps(tab, indices_to_read_from, sizeof(float));
...
distance_four_codes_avx2(
    ...
    // computed distances
    float& result0, float& result1, float& result2, float& result3) {
  distance_four_codes_generic<PQDecoderT>(M, nbits, sim_table, code0, code1, code2, code3, result0,
...
distance_four_codes_avx2(
    ...
    float& result0, float& result1, float& result2, float& result3) {
  if (M == 4) {
    distance_four_codes_avx2_pqdecoder8_m4(sim_table, code0, code1, code2, code3, result0, result1,
                                           result2, result3);
    return;
  }
  if (M == 8) {
    distance_four_codes_avx2_pqdecoder8_m8(sim_table, code0, code1, code2, code3, result0, result1,
                                           result2, result3);
    return;
  }
  ...
  if (pqM16 > 0) {
    ...
    for (m = 0; m < pqM16 * 16; m += 16) {
      ...
      for (intptr_t j = 0; j < N; j++) {
        // move high 8 uint8 to low ones
        const __m128i mm2 = _mm_unpackhi_epi64(mm1[j], _mm_setzero_si128());
